...
custom_packages() {
    ...
    cd packages
    cp -f ${make_path}/config/imagebuilder/packages/*.ipk . 2>/dev/null || true
...
rebuild_firmware() {
    ...
    my_packages="\
        ...
        which dnsmasq-full ip-full iptables-nft v2ray-geoip v2ray-geosite wpad-openssl wwan xfs-fsck xfs-mkfs xz xz-utils ziptool zoneinfo-asia zoneinfo-core zstd \
        kmod-tun \
        kmod-nft-tproxy kmod-nft-socket \
        kmod-tcp-bbr kmod-nft-offload \
        htop irqbalance \
        tcpdump ethtool-full \
        \
